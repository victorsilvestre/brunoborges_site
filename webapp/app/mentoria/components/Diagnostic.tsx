const diagnostics = [
    ...
    },
    {
        ...
        body: "Entra em operações com dúvida e percebe, muitas vezes tarde demais, que não havia entendido corretamente o contexto.",
    ...
    },
    {
        ...
        body: "Tem bons dias ou boas operações, mas ainda não construiu um processo claro que consiga aplicar com consistência.",
        ...
        <section className="w-full bg-[var(--m-subtle)] border-b border-[var(--m-border)] py-16 lg:py-24">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {diagnostics.map((d) => (
                        <div key={d.tag} className="bg-white p-6 rounded-lg border border-[var(--m-border)] flex flex-col justify-between hover:border-[var(--m-bear)] transition-all group">
                            <div>
                                <div className="flex items-center justify-between mb-4">
                                    <span className="font-[family-name:var(--m-font-mono)] text-sm font-bold text-[var(--m-bear)] bg-[var(--m-bear-bg)] px-2 py-0.5 rounded border border-[var(--m-bear-border)]">{d.tag}</span>
                                    <span className="font-[family-name:var(--m-font-mono)] text-xs text-[var(--m-caption)]">{d.roman}</span>
                                </div>
                ...
                <div className="mt-10 p-6 sm:p-8 bg-[var(--m-dark)] text-white rounded-lg border border-slate-700 flex flex-col md:flex-row items-center justify-between gap-6 shadow">
                    ...
                    <a className="flex-shrink-0 px-6 py-3.5 bg-[var(--m-bull)] hover:bg-[var(--m-bull-light)] text-white font-[family-name:var(--m-font-mono)] text-xs uppercase tracking-wider font-bold rounded transition-all shadow" href="#inscricao">
                        Garantir Minha Vaga
                    </a>
                </div>
            </div>
